...
async function carregarUsuario(usuario) {
  const url = `${GITHUB_BASE}dados_${usuario}.js?v=${Date.now()}`;
  if (window.DB) delete window.DB;
  try {
    const resp = await fetch(url);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    // const/let não vazam para window dentro de eval — convertemos para window.DB
    const codigo = (await resp.text()).replace(/^\s*(const|let|var)\s+DB\s*=/, 'window.DB =');
    eval(codigo);
  } catch (e) {
  ...
  if (!window.DB) {
    alert(`O arquivo dados_${usuario}.js não definiu o banco de dados corretamente.`);
    return;
  }
