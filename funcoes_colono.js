...
// (substitui o download de dados_colono.js do original)
// ----------------------------------------------------------
...
function salvarDados() {
  const dbAtualizado = {
    ...
    conclusaoComposta: {
      ...
      quantidades: DB.conclusaoComposta.quantidades, // estrutura fixa, não muda
  ...
  try {
    ...
    mostrarToast('💾 Banco salvo no navegador!', '#1a3a1a');
    atualizarIndicadorStorage();
  } catch (e) {
    mostrarToast('⚠️ Erro ao salvar: ' + e.message, '#7a1a1a');
  }
}

// ----------------------------------------------------------
// EXPORTAR BACKUP JS
// (equivalente ao salvarDados() original — baixa o arquivo)
// ----------------------------------------------------------

function exportarJS() {
  const dbAtualizado = JSON.parse(localStorage.getItem('colonoDB') || JSON.stringify(DB_PADRAO));
  const conteudo =
`// ============================================================
...
const DB_PADRAO = ${JSON.stringify(dbAtualizado, null, 2)};
`;
