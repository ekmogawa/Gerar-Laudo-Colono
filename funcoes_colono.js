...
function inicializar() {
  ...
  inicializarTokenGitHub(); // descriptografa token se config.js tiver tokenCriptografado
}
...
// Prioridade: config.js com token criptografado > localStorage (popup manual)
// O token descriptografado fica apenas em memória (sessionStorage como cache
// de sessão — apagado ao fechar o browser).
// ----------------------------------------------------------
...
function lerConfigGitHub() {
  ...
  // config.js presente mas sem token criptografado (placeholder)
  if (!GITHUB_CONFIG.tokenCriptografado) return GITHUB_CONFIG;
  // Retorna config sem token — o token será injetado de forma assíncrona
...
// Descriptografa o token usando a senha fornecida
async function descriptografarToken(senha) {
  try {
    const fromB64 = b64 => Uint8Array.from(atob(b64), c => c.charCodeAt(0));
    const salt    = fromB64(GITHUB_CONFIG.salt);
    const iv      = fromB64(GITHUB_CONFIG.iv);
    const cifrado = fromB64(GITHUB_CONFIG.tokenCriptografado);

    const keyMaterial = await crypto.subtle.importKey(
      'raw', new TextEncoder().encode(senha),
...
// Chamado na inicialização: pede a senha uma vez por sessão
async function inicializarTokenGitHub() {
  if (typeof GITHUB_CONFIG === 'undefined' || !GITHUB_CONFIG.tokenCriptografado) return;

  // Já tem token na sessão atual?
  ...
  while (tentativas < 3) {
    const senha = prompt(
      tentativas === 0
        ? '🔐 Digite a senha para ativar o salvamento no GitHub:'
        : `❌ Senha incorreta. Tente novamente (tentativa ${tentativas + 1}/3):`
    );
    if (senha === null) break; // usuário cancelou

    const token = await descriptografarToken(senha);
    if (token) {
  ...
  if (tentativas === 3) mostrarToast('⚠️ Senha incorreta 3x. GitHub desativado nesta sessão.', '#7a1a1a');
  atualizarStatusGitHub();
}
...
async function salvarNoGitHub(dbAtualizado) {
  ...
  // Obtém o token: sessionStorage (criptografado) ou direto da config (fallback)
  const token = sessionStorage.getItem('colono_github_token') || c.token;

  if (!token || !c.owner || !c.repo) {
    mostrarToast('⚠️ GitHub não configurado ou sessão expirada. Recarregue a página.', '#7a4000');
    return false;
  }
