...
function carregarUsuario(usuario) {
  const url = `${GITHUB_BASE}dados_${usuario}.js`;

  // Remove DB anterior se existir
  const scriptAnterior = document.getElementById('script-dados-usuario');
  if (scriptAnterior) scriptAnterior.remove();
  if (window.DB) delete window.DB;

  const script = document.createElement('script');
  script.id  = 'script-dados-usuario';
  script.src = url + '?v=' + Date.now(); // evita cache
  script.onload = () => {
    if (!window.DB) {
      alert(`Arquivo dados_${usuario}.js não encontrado no GitHub.\nVerifique se o arquivo foi enviado corretamente.`);
      return;
    }
  ...
  script.onerror = () => {
    alert(`Não foi possível carregar dados_${usuario}.js.\nVerifique sua conexão ou se o arquivo existe no GitHub.`);
  };
...
function salvarDados() {
  ...
  const nomeArquivo = `dados_${usuarioAtivo}.js`;
  const conteudo =
`// ============================================================
// BANCO DE DADOS — Gerar Laudo Colonoscopia
// Usuário: ${usuarioAtivo}
// Salvo em: ${new Date().toLocaleString('pt-BR')}
// ============================================================

const DB = ${JSON.stringify(dbAtualizado, null, 2)};
`;
