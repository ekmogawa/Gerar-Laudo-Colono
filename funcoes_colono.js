...
async function inicializarTokenGitHub() {
  ...
  // Sem token criptografado: verifica se há token direto (não recomendado em repo público)
  if (!c.tokenCriptografado) {
    atualizarStatusGitHub();
    return;
  }

  // Já descriptografado nesta sessão
  if (sessionStorage.getItem('colono_github_token')) {
    atualizarStatusGitHub();
    return;
  }

  // Pede a senha (máx 3 tentativas)
  var tentativas = 0;
  while (tentativas < 3) {
    var msg   = tentativas === 0 ? '🔐 Digite a senha para ativar o GitHub:'
                                 : '❌ Senha incorreta. Tentativa ' + (tentativas + 1) + '/3:';
    var senha = prompt(msg);
    if (senha === null) break; // cancelou
